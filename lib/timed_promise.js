...
class TimedPromise extends Promise
{
    ...
    timeout( timeout_ms, reason = 'timeout', catchable_by_parent = false )
    {
        ...
        if( timed_promise && timed_promise.pending && this.remaining() > timeout_ms )
        {
            ...
            timed_promise.timeout = Date.now() + timeout_ms;
    ...
    elapsed()
    {
        return Date.now() - this._timed_promise.created;
    }
    ...
    remaining()
    {
        ...
        if( timed_promise.pending )
        {
            return timed_promise.timeout === Infinity ? Infinity : Math.max( 0, timed_promise.timeout - Date.now() );
        }
        else{ return 0 }
    }
    ...
    get settled()
    {
        return !this._timed_promise.pending;
    }
}
